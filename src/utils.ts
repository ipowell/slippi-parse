...
export type GameParser = (game: SlippiGame) => [number, number][];
...
export class ClipFinder {
    ...
    constructor(parser: GameParser,
        filename: string, leadingFrames: number = 240, trailingFrames: number = 159) {
        this.parser = parser
    ...
    parseGame(game: SlippiGame) {
        ...
        this.clips.push(...getQueueData(game.getFilePath(), game, frames, this.leadingFrames, this.trailingFrames))
    }
...
export function didHit(frame: FrameEntryType, attacker: PlayerType, attackee: PlayerType) {
    return isAlive(frame, attacker) && isAlive(frame, attackee) &&
...
export function tookXDamageThisFrame(frames: FramesType, frame: FrameEntryType, player: PlayerType, damage: number) {
    const nextFrame = frames[frame.frame + 1]
    if (nextFrame == null || nextFrame.players[player.playerIndex] == null) {
        return false
    }
...
export function wasLastHitBy(frame: FrameEntryType, attacker: PlayerType, attackee: PlayerType) {
...
export function printGame(game: SlippiGame) {
    for (let i = 0; i < 6000; i++) { //game.getMetadata().lastFrame!; i++) {
        ...
        game.getSettings().players.forEach((player: PlayerType, index: number, array: PlayerType[]) => {
            ...
            if (isAlive(frame, player)) {
                ...
                let actionStateId = frame.players[index].post.actionStateId
                info = info + ", " + name + " state " + state + " " + percent.toPrecision(3) + "% in " + actionStateId
            } else {
...
export function getPlayerWithConnectCode(game: SlippiGame, code: string): (PlayerType | null) {
    return game.getSettings().players.find((player: PlayerType) => player.connectCode == code)
}